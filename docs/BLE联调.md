...
本文用于验证“STM32 体测结果 → UART/BLE 透传 → Android Chrome 看板”的单向上报链路。
...
接线只需 `STM32 PA2/USART2_TX → ESP32 GPIO16/RX2`，并连接 `GND → GND`；ESP32
建议独立 USB 供电。网关广播 `HealthTerminal-Gateway`，提供 FFE0 Primary Service
和 FFE1 Notify 特征，详细烧录步骤见
...
4. 点击“连接 BLE 设备”。网页使用 `acceptAllDevices: true`，设备选择器可能显示附近其他
   ...
   不要修改 STM32 帧协议。
5. 在终端完成一次测量并进入结果页。结果状态入口只发送一次 59 字节 RECORD 帧。
