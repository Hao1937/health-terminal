...
/* 使能某 GPIO 端口时钟（board_gpio_init 已开 A/B/C，这里再做一次保证自洽、幂等） */
...
static inline void sck_high(const hx711_t *h) { HAL_GPIO_WritePin(h->sck_port, h->sck_pin, GPIO_PIN_SET); }
static inline void sck_low (const hx711_t *h) { HAL_GPIO_WritePin(h->sck_port, h->sck_pin, GPIO_PIN_RESET); }
...
{
    return (HAL_GPIO_ReadPin(h->dout_port, h->dout_pin) == GPIO_PIN_RESET) ? 1 : 0;
}
...
int hx711_read_raw(hx711_t *h, int32_t *out, uint32_t timeout_ms)
{
    ...
    /* 关中断，避免 SCK 高相位被拉长(>60us)误触发掉电；用 PRIMASK 保存恢复，可嵌套安全 */
    ...
    for (int i = 0; i < 24; ++i) {
        sck_high(h);
        delay_us(1);
        uint32_t bit = (HAL_GPIO_ReadPin(h->dout_port, h->dout_pin) == GPIO_PIN_SET) ? 1u : 0u;
        value = (value << 1) | bit;
        sck_low(h);
        delay_us(1);
    }
