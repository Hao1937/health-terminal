 ...
 * @owner   王宇浩（组长）
 *
 * MAX30102 心率血氧（I2C 0x57）。驱动只负责「配置 + 从 FIFO 取一段 RED/IR
 * 原始样本」， 心率/血氧解算交给 algorithms/spo2_hr（与硬件解耦、可在 PC
 * 回放测试）。
 *
...
hs_status_t max30102_init(void) {
  ...
  /* SPO2_CONFIG：ADC 量程 4096nA(0b01<<5)、采样率 400Hz(0b011<<2)、脉宽
   * 411us/18bit(0b11) */
...
hs_status_t max30102_measure(hs_sample_t *out) {
  ...
  while (count < TARGET_SAMPLES) {
    ...
    for (int k = 0; k < avail && count < TARGET_SAMPLES; ++k) {
      ...
      if (i2c_mem_read(MAX30102_ADDR, REG_FIFO_DATA, d, BYTES_PER_SAMPLE) !=
          0) {
        return HS_TIMEOUT;
      }
      /* 每通道 18bit，高位在前；SpO2 模式 FIFO 顺序为 RED 后 IR */
      uint32_t red =
          (((uint32_t)d[0] << 16) | ((uint32_t)d[1] << 8) | d[2]) & 0x03FFFFu;
      uint32_t ir =
          (((uint32_t)d[3] << 16) | ((uint32_t)d[4] << 8) | d[5]) & 0x03FFFFu;
