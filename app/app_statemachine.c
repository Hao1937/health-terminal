...
{
    switch (item) {
        case HS_ITEM_HEIGHT:   g_current_record.height_mm = s->primary; break;
        case HS_ITEM_WEIGHT:   g_current_record.weight_g = s->primary; break;
        case HS_ITEM_HR_SPO2:
            g_current_record.heart_rate_bpm = s->primary;
            g_current_record.spo2_x10 = s->secondary; break;
        case HS_ITEM_BALANCE:  g_current_record.balance_x10 = s->primary; break;
        case HS_ITEM_GRIP:     g_current_record.grip_kg_x10 = s->primary; break;
        case HS_ITEM_REACTION: g_current_record.reaction_ms = s->primary; break;
        default: break;
    }
}
...
static void measure_all(void)
{
    for (const hs_sensor_t *s = g_hs_registry; s->measure != NULL; ++s) {
        ...
        printf("[measure] %-9s (%s) -> %s\r\n", s->name, s->owner, hs_status_str(st));
...
void app_init(void)
{
    ...
    printf("[boot] I2C probe: OLED=%d MPU6050=%d MAX30102=%d\r\n",
           i2c_probe(I2C_ADDR_OLED), i2c_probe(I2C_ADDR_MPU6050), i2c_probe(I2C_ADDR_MAX30102));
}
