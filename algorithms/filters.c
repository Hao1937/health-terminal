...
int32_t median_push(median_filter_t *f, int32_t sample) {
  /* 契约要求先调用 median_init()；window==0 时按 1
   * 处理，避免除零与返回未初始化值 */
  if (f->window < 1) f->window = 1;
...
int32_t movavg_push(movavg_filter_t *f, int32_t sample) {
  /* 契约要求先调用 movavg_init()；window==0 时按 1 处理，避免除零 */
  if (f->window < 1) f->window = 1;
...
float comp_update(comp_filter_t *f, float acc_angle, float gyro_rate,
                  float dt) {
  if (!f->inited) {
