...
- STM32 主动上传最新综合记录；
- 网页发送 `PING`、`GET CURRENT`、`GET HISTORY`，也能逐项写入当前记录；
- STM32 返回当前缓存或 Flash 中最多 76 条历史记录；
- 浏览器本地保留最近 200 条去重记录，并支持 CSV 导出；
- BLE 意外断开后尝试三次自动重连；
- 对异常长度和 CRC 错误进行帧头重同步。

写入协议使用 `SET H/W/F/HR/O/B/G/R <定点整数>` 修改身高、体重、体脂率、
心率、血氧、平衡、握力和反应时间。网页每写一项都等待设备返回 `OK <字段>`；
`APPLY` 只更新当前记录，`SAVE` 会追加一条设备 Flash 历史。BMI 和综合评分始终
由 STM32 重算，空白输入保持当前值。
...
6. 如需手机补录或修改，填写“手机写入设备”，再选择“写入当前值”或“写入并保存历史”；
7. 完成队友及本人模块的测量，也可执行终端的 `Score/Upload`；
8. 对照 OLED、网页当前值和历史表，必要时导出 CSV。

网页不会远程启动传感器或删除 Flash 历史；手机写入只修改明确填写的字段。
